...
**Status reporting** — call `report_status` with your `agent_id`, `agent_type` ("opencode"), `workspace_id`, and `status` (one of: pending, thinking, planning, running, editing, testing, waiting, blocked, paused, handoff, succeeded, failed, cancelled). Never report "stuck" or "orphaned".
